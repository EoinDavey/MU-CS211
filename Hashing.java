...
//create the hash table
    static String[] array = new String[216553]; //make sure your String array is big enough to hold all the data
...
//print out the total number of attempts to find the correct slot
     ...
     public static int getDoubleHashKey(String word){
        ...
        for(int i = 0; i < word.length(); i++){
            hash = (hash*33) ^ (((long)word.charAt(i))<<2);
        }
     ...
     public static long inHash(long key){
         key = ~key + (key << 21);
         key = key ^ (key >>> 24);
         key = key + (key << 3) + (key << 8);
         key = key ^ (key >>> 14);
     ...
     public static int cgetHashKey(String word){
         int t = Arrays.binarySearch(array, word);
         return t;
     }
...
//print out the information for the last 1,000 words only, otherwise it takes quite long and gets annoying
      ...
      public static void fillDoubleHash(){
         ...
         System.out.println("The total number of collisions was "+ totalcollisions + " mx: "+ mxcol + ": " + array[mx]);
      }
